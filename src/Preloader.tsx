...
function Preloader() {
  ...
  useEffect(() => {
    ...
    const updateProgress = (nextProgress: number) => {
      ...
      latestProgress.current = clamped;
      setProgress(clamped);
    };
    ...
    const updateFromCount = () => {
      resourcesLoaded += 1;
      updateProgress(Math.min(95, (resourcesLoaded / total) * 100));
    };
    ...
    if (images.length === 0) {
      ...
      images.forEach((image) => {
        ...
        image.addEventListener("load", updateFromCount, { once: true });
        image.addEventListener("error", updateFromCount, { once: true });
      });
    }
    ...
    const indeterminateTimer = window.setTimeout(() => {
      if (latestProgress.current < 10) setIndeterminate(true);
    }, 1500);
    ...
    return () => {
      ...
      images.forEach((image) => {
        image.removeEventListener("load", updateFromCount);
        image.removeEventListener("error", updateFromCount);
      });
    };
  }, []);
  ...
  return (
    ...
    >
      <svg className="preloader-icon" viewBox="0 0 68.21 68.21" role="img">
        <title>Loading</title>
        <circle cx="34.1" cy="34.1" r="34.1" fill="#f05323" />
        <circle
        ...
        />
        <g className="preloader-blades" fill="#f8f8f6">
